...
select budget from departments where budget = (select max(budget) from departments where budget < (select max(budget) from departments));
select name, lastname from employees where department = (select department from departments where budget = (select min(budget) from departments));
select name, lastname from employees where city = 'Almaty' UNION select name, lastname from customers where city = 'Almaty';
select code from departments where budget > 60000 ORDER BY budget ASC, code DESC; 
